...
export default function Home() {
  const [inputs, setInputs] = useState<FunnelCountInputs>(DEFAULT_FUNNEL_INPUTS);
  const [reportStart, setReportStart] = useState("");
  const [reportEnd, setReportEnd] = useState("");

  useLayoutEffect(() => {
    const { start, end } = defaultLast30DayWindow();
  ...
  const connectionVolumeStatusValue: FunnelStatus = useMemo(
    () => (reportWeeks > 0 ? connectionVolumeStatus(connectionRequestsPerWeekValue) : "na"),
  ...
  /** Pace target for the window: (days ÷ 7) × 150 connection requests/week. */
  ...
  /** Hide global priority until the user has entered at least one connection request. */
  const hasMeaningfulInput = counts.sent > 0;
  const showPrioritySidebar =
    priority !== null && hasMeaningfulInput;
  ...
  /** Marketing or overall composite is green, but connection-request pace is not — surface volume in the priority card. */
      ...
      <main className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-4 py-10 sm:px-6">
        <header className="flex flex-col gap-2">
          ...
          <p className="max-w-3xl text-sm leading-6 text-zinc-600">
            Enter your numbers and get instant KPI feedback. Green means you’re
            on track, yellow is within 20% of target, red is the constraint to
            fix first.
          </p>
        </header>
